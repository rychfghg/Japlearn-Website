import { BarChart3, ChevronDown, Download, FileBarChart, ListFilter, Users } from "lucide-react";
...
export default function ReportsPage() {
  ...
  return <section className="reports-page">
    <div className="reports-title"><div><small>LEARNING INTELLIGENCE</small><h1>Progress reports</h1><p>Turn saved attempts into a clear view of growth, strengths, and next steps.</p></div><span><BarChart3 /> Account-synced results</span></div>
    ...
    {view === "student" ? <StudentScoreReport students={students} /> : <div className="reports-masterlist">
      ...
      <div className="reports-filterbar"><label className="designed-select"><span><Users />Learners</span><div><select aria-label="Students to include" disabled={busy} value={scope} onChange={e => setScope(e.target.value)}><option value="">All students ({students.length})</option>{students.map(s => <option key={s.email} value={s.email}>{s.fname} {s.lname}</option>)}</select><ChevronDown /></div></label><button className="report-build-button" disabled={busy || !students.length || !sections.length} onClick={build}><ListFilter />{busy ? "Building…" : "Build report"}</button>{cards && <button className="report-export-button" onClick={() => downloadCsv("japlearn-masterlist.csv", scorecardsToCsv(cards, sections))}><Download />Export CSV</button>}</div>
      <details className="report-feedback-details"><summary><span><ListFilter />Included activity groups</span><em>{sections.length} selected</em></summary><div className="chip-row">{REPORT_SECTIONS.map(s => <button disabled={busy} key={s.key} className={`chip ${sections.includes(s.key) ? "on" : ""}`} onClick={() => setSections(current => current.includes(s.key) ? current.filter(k => k !== s.key) : [...current, s.key])}>{s.label}</button>)}</div></details>
